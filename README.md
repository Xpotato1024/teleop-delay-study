...
MATLAB/Simulinkを用いて、遠隔位置指令系の固定通信遅延に対するゼロ次ホールド（ZOH）と定速度予測（CV）を比較し、追従誤差の低減範囲と悪化条件を解析したシミュレーション研究です。再現用コード、標準40条件の解析、図表、および最終レポートを収録しています。

2026年度「シミュレーション工学・演習」の最終課題として完了済みです。提出済み最終レポートに対応する再現用XeLaTeX原稿と生成手順は`report/`以下に保存しています。
