...
bufferCapacity = teleopdelay.config.communication_buffer_capacity();
close_if_loaded(modelName);
new_system(modelName);
cleanup = onCleanup(@() close_if_loaded(modelName));
set_param(modelName, "Solver", "ode4", "FixedStep", "0.01", "StopTime", "10", ...
...
chart.Script = communication_function_code(bufferCapacity);
...
configure_port([modelName '/zoh_command_xy_m'], "-1", "m");
configure_port([modelName '/cv_command_xy_m'], "-1", "m");
configure_scalar_port([modelName '/packet_timestamp_s'], "-1", "s", "double");
configure_scalar_port([modelName '/packet_age_s'], "-1", "s", "double");
configure_scalar_port([modelName '/packet_valid'], "-1", "", "boolean");
...
function code = communication_function_code(bufferCapacity)
capacity = string(bufferCapacity);
lines = [ ...
    ...
    "buffer_capacity = " + capacity + ".0;"; ...
    "if isempty(packet_count)"; ...
    "    next_sample = 0.0; write_index = 1.0; packet_count = 0.0;"; ...
    "    packet_timestamps = -ones(1,buffer_capacity); packet_positions = zeros(buffer_capacity,2); packet_velocities = zeros(buffer_capacity,2);"; ...
    "end"; ...
    "tol = 1.0e-10;"; ...
    ...
    "    write_index = write_index + 1.0; if write_index > buffer_capacity, write_index = 1.0; end; packet_count = min(packet_count + 1.0, buffer_capacity);"; ...
...
function configure_scalar_port(blockPath, sampleTime, unit, dataType)
set_param(blockPath, "PortDimensions", "1", "OutDataTypeStr", dataType, "Unit", unit, "SampleTime", sampleTime);
end
