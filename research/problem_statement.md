...
定速度予測補償は、目標軌道が局所的にほぼ一定速度で変化し、通信遅延が過大でない条件では追従誤差を低減すると考える。一方、軌道の曲率変化が大きい条件や遅延が大きい条件では、速度一定という仮定が崩れ、補償効果が小さくなる、または補償なしより悪化する可能性がある。これは検証前の仮説であり、結果ではない。
...
この初期スケルトンでは、科学モデル本体、ZOH、定速度予測、RK4、実験結果、ランダム軌道、実機検証、Simulinkモデル、Python実装、外部依存パッケージを対象外とする。
